...
3、运行生成报告：pytest 脚本.py -- html=respote.html
...
def test_register_001():
    data = {"mobilephone": "1802233445", "pwd": "123456"}
    r =requests.post(url,data=data)
    assert r.json()['msg'] == '手机号码格式不正确'

def test_register_002():
    data = {"mobilephone": "18022334455", "pwd": "1234"}
    r =requests.post(url,data=data)
    assert r.json()['msg'] == "密码长度必须为6~18"

def test_register_003():
    data = {"mobilephone": "18022334455", "pwd": "1234567890111111111"}
    r =requests.post(url,data=data)
    assert r.json()['msg'] == "密码长度必须为6~18"